<?php
class Bluga_HTTP_Request_Adapter_Phpsocket_Socket {
 ...
 */
class Bluga_HTTP_Request_Adapter_Phpsocket extends Bluga_HTTP_Request_Adapter {
    ...
    public function sendRequest() {
        ...
        if (!is_resource($handle)) {
            throw new Bluga_HTTP_Request_Exception("Couldn't connection to host using Phpsocket Adapter, fsockopen errors($errstr,$errno)");
        }
        stream_set_timeout($handle,10);

        $this->_stream = new Bluga_HTTP_Request_Adapter_Phpsocket_Socket($handle);
        ...
        $response = new Bluga_HTTP_Request_Response(
            $details,$this->body,new Bluga_HTTP_Request_Headers($this->headers),$this->cookies);
        return $response;
    }
    ...
    */
    private function _decodeGzip($data)
    {
        ...
        if ($method != 8) {
            throw new Bluga_HTTP_Request_Exception('_decodeGzip(): unknown compression method');
        }
        
        $flags = ord(substr($data, 3, 1));

        if ($flags & 224) {
            throw new Bluga_HTTP_Request_Exception('_decodeGzip(): reserved bits are set');
        }
        ...
        if ($flags & 4) {
            if ($length - $headerLength - 2 < 8) {
                throw new Bluga_HTTP_Request_Exception('_decodeGzip(): data too short');
            }
        ...
        if ($flags & 1) {
            ...
            if ($crcReal != $crcStored[1]) {
                throw new Bluga_HTTP_Request_Exception('_decodeGzip(): header CRC check failed');
            }
            $headerLength += 2;
        }
        ...
        if (false === $unpacked) {
            throw new Bluga_HTTP_Request_Exception('_decodeGzip(): gzinflate() call failed');
        } elseif ($dataSize != strlen($unpacked)) {
            throw new Bluga_HTTP_Request_Exception('_decodeGzip(): data size check failed');
        } elseif ($dataCrc != crc32($unpacked)) {
            throw new Bluga_HTTP_Request_Exception('_decodeGzip(): data CRC check failed');
        }
        return $unpacked;
    }
